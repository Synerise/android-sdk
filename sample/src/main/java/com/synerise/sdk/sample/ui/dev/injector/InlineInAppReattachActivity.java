...
public class InlineInAppReattachActivity extends BaseActivity {

    private static final String PLACEMENT_KEY = "single";
    private final FrameLayout.LayoutParams viewLayoutParams = new FrameLayout.LayoutParams(
    ...
    public static Intent createIntent(Context context) {
        return new Intent(context, InlineInAppReattachActivity.class);
    }

    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_inline_inapp_reattach);
        ToolbarHelper.setUpChildToolbar(this, R.string.inline_inapp_reattach);
        ...
        container.addView(inlineInAppView, viewLayoutParams);
        appendLog("init", "created with key=" + PLACEMENT_KEY);

        Injector.setOnInlineInAppListener(createGlobalListener());

        findViewById(R.id.btn_detach).setOnClickListener(v -> detachView());
        findViewById(R.id.btn_attach).setOnClickListener(v -> attachView());
        ...
        findViewById(R.id.btn_release).setOnClickListener(v -> {
            appendLog("action", "release() called");
    ...
    private void detachView() {
        if (inlineInAppView.getParent() == null) {
            appendLog("action", "detach skipped — already detached");
            return;
        }
        appendLog("action", "removeView() — temporary detach (Activity alive)");
        container.removeView(inlineInAppView);
    }

    private void attachView() {
        if (inlineInAppView.getParent() != null) {
            appendLog("action", "attach skipped — already attached");
            return;
        }
        appendLog("action", "addView() — re-attach (expect same in-app, no AUTOMATIC re-trigger)");
        container.addView(inlineInAppView, viewLayoutParams);
    }
