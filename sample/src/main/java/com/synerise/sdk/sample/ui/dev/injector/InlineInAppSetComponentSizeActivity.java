...
public class InlineInAppSetComponentSizeActivity extends BaseActivity {

    private static final String PLACEMENT_KEY = "autoresize";
    ...
    public static Intent createIntent(Context context) {
        return new Intent(context, InlineInAppSetComponentSizeActivity.class);
    }

    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_inline_inapp_set_component_size);
        ToolbarHelper.setUpChildToolbar(this, R.string.inline_inapp_set_component_size);
    ...
    private OnInlineInAppViewListener createListener() {
        return new OnInlineInAppViewListener() {
            ...
            public void onSizeChanged(InlineInAppMessageData data, InlineInAppSize size) {
                appendLog("onSizeChanged", describeAllSizes(size));
            }
        };
    }

    private String describeAllSizes(InlineInAppSize size) {
        return "px=" + size.getWidthPx() + "x" + size.getHeightPx()
                + " | dp=" + size.getWidthDp() + "x" + size.getHeightDp()
                + " | screen=" + toPercent(size.getWidthScreenRatio())
                + "x" + toPercent(size.getHeightScreenRatio());
    }
