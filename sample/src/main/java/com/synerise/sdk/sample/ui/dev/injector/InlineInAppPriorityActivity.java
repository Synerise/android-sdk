...
public class InlineInAppPriorityActivity extends BaseActivity {

    private static final String PLACEMENT_KEY = "priority";
    ...
    public static Intent createIntent(Context context) {
        return new Intent(context, InlineInAppPriorityActivity.class);
    }

    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_inline_inapp_priority);
        ToolbarHelper.setUpChildToolbar(this, R.string.inline_inapp_priority);
        ...
        findViewById(R.id.btn_send_event).setOnClickListener(v -> {
            String eventName = "inline.priority";
            appendLog("action", "sending event: " + eventName);
            CustomEvent event = new CustomEvent(eventName, "Priority trigger event");
            Tracker.send(event);
        });

        findViewById(R.id.btn_send_event_high).setOnClickListener(v -> {
            String eventName = "inline.priorityhigh";
            appendLog("action", "sending event: " + eventName);
            CustomEvent event = new CustomEvent(eventName, "Priority high trigger");
            Tracker.send(event);
        });
    }
