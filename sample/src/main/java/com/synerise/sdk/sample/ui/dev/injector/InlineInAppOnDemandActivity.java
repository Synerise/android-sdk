...
public class InlineInAppOnDemandActivity extends BaseActivity {
    ...
    public static Intent createIntent(Context context) {
        return new Intent(context, InlineInAppOnDemandActivity.class);
    }

    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_inline_inapp_on_demand);
        ToolbarHelper.setUpChildToolbar(this, R.string.inline_inapp_on_demand);
        ...
        findViewById(R.id.btn_send_event).setOnClickListener(v -> {
            String eventName = "inline.trigger";
            appendLog("action", "sending event: " + eventName);
            CustomEvent event = new CustomEvent(eventName, "Inline trigger event",
                    new TrackerParams.Builder().add("my_source", "on_demand_screen").build());
            Tracker.send(event);
        });
    }
